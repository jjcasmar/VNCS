...
void Generator::operator()()
{
    ...
    // We only have a point cloud set. We could even load this directly in Python
    {
        logger->info("Loading coarse PC from {}", m_coarseMeshPath);
        std::vector<VNCS::Space3D::Point> coarsePoints;
    ...
    m_exportFunction({fineMeshPoints, fineMeshTetra}, "fineTest.vtu");
