...
VNCS::Sampler<VNCS::Space1D> GenerateSampler::operator()(
    const SamplingPointsExporter::FineSamplerDefinition &samplerDefinition)
{
    using SamplerDefinition = SamplingPointsExporter::FineSamplerDefinition;

    VNCS::Sampler<VNCS::Space1D> sampler;
    sampler.w = samplerDefinition.weight;
    sampler.a = 0.0;

    auto &baseShapeFunctions = sampler.fineShapeFunctions;

    VNCS::ShapeFunction<VNCS::Space1D> shapeFunction;
    shapeFunction.nodeIndex = samplerDefinition.pointHandle;
    shapeFunction.v = 1.0;
    baseShapeFunctions.push_back(shapeFunction);
