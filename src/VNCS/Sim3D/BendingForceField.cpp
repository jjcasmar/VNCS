...
namespace
{
auto make_eigen = [](const auto &v) -> Eigen::Vector3d { return {v[0], v[1], v[2]}; };
auto view_eigen = [](const auto &v) -> Eigen::Map<const Eigen::Vector3d> {
    return Eigen::Map<const Eigen::Vector3d>(v.data());
};
}  // namespace

VNCS::Sim3D::BendingForceField::BendingForceField()
    : Inherit()
{
}

void VNCS::Sim3D::BendingForceField::init()
{
    Inherit::init();
    ...
    for (const auto &vertex : boost::make_iterator_range(boost::vertices(m_edgeMesh))) {
        ...
        for (auto edge0 = outEdgesBegin; edge0 != outEdgesEnd; ++edge0) {
            for (auto edge1 = edge0 + 1; edge1 != outEdgesEnd; ++edge1) {
                ...
                const auto du =
                    (Eigen::Map<const Eigen::Vector3d>(xLeft.data()) - Eigen::Map<const Eigen::Vector3d>(xRight.data()))
                        .norm();

                m_elements.push_back({leftId, centerId, rightId, du});
            }
        }
    }
}

VNCS::Space2D::Real VNCS::Sim3D::BendingForceField::getPotentialEnergy(const sofa::core::MechanicalParams *,
                                                                       const DataVecCoord &xVec) const
{
    ...
    for (const auto &element : m_elements) {
        ...
        const Eigen::Vector3d d1 = (xRight - xCenter).normalized();
        const Eigen::Vector3d d2 = (xLeft - xCenter).normalized();
...
void VNCS::Sim3D::BendingForceField::addForce(const sofa::core::MechanicalParams *mparams,
                                              DataVecDeriv &fVec,
                                              ...
                                              const DataVecDeriv &vVec)
{
    ...
    for (const auto &element : m_elements) {
        ...
        const auto xRestPoseLeft = Eigen::Map<const Eigen::Vector3d>(&xRestPose[leftIndex][0]);
        const auto xRestPoseCenter = Eigen::Map<const Eigen::Vector3d>(&xRestPose[centerIndex][0]);
        const auto xRestPoseRight = Eigen::Map<const Eigen::Vector3d>(&xRestPose[rightIndex][0]);

        const Eigen::Vector3d xLeft = Eigen::Map<const Eigen::Vector3d>(&x[leftIndex][0]) + xRestPoseLeft;
        const Eigen::Vector3d xCenter = Eigen::Map<const Eigen::Vector3d>(&x[centerIndex][0]) + xRestPoseCenter;
        const Eigen::Vector3d xRight = Eigen::Map<const Eigen::Vector3d>(&x[rightIndex][0]) + xRestPoseRight;

        Eigen::Vector3d d1 = (xRight - xCenter);
        Eigen::Vector3d d2 = (xLeft - xCenter);
        ...
        const Eigen::Matrix3d P1 = Eigen::Matrix3d::Identity() - d1 * d1.transpose();
        const Eigen::Matrix3d P2 = Eigen::Matrix3d::Identity() - d2 * d2.transpose();
        ...
        if (theta != 0.0)  // otherwise error due to division by 0 due to sin(theta), but forces are 0 anyway
        {
            // stuff related with forces
            {
                ...
                Eigen::Vector3d fx1 = common / l1 * P1 * d2;
                Eigen::Vector3d fx2 = common / l2 * P2 * d1;

                auto make_deriv = [](const Eigen::Vector3d &v) {
                    return VNCS::Space3D::VecType::Deriv(v[0], v[1], v[2]);
                };

                f[centerIndex] -= make_deriv((fx1 + fx2).head<3>());
                f[leftIndex] += make_deriv((fx2).head<3>());
                f[rightIndex] += make_deriv((fx1).head<3>());
            }
            ...
            // stuff related with hessians
            {
                ...
                const Eigen::Vector3d P1d2 = P1 * d2;
                const Eigen::Vector3d P2d1 = P2 * d1;

                // lagrangian - lagrangian

                const Eigen::Matrix3d P1d2d1t = P1d2 * d1.transpose();
                const Eigen::Matrix3d P2d1d2t = P2d1 * d2.transpose();

                const VNCS::Real commonLL = 2. * m_bendingStiffness / (duLeftRight * sinTheta);

                Eigen::Matrix3d jacobiansLL[3][3];
                ...
                Eigen::Matrix<VNCS::Real, 9, 9> hessian;

                // Diagonal terms
                hessian.block<3, 3>(0, 0) = jacobiansLL[0][0];
                hessian.block<3, 3>(3, 3) = jacobiansLL[1][1];
                hessian.block<3, 3>(6, 6) = jacobiansLL[2][2];

                // Upper off diagonal
                hessian.block<3, 3>(0, 3) = jacobiansLL[0][1];
                hessian.block<3, 3>(0, 6) = jacobiansLL[0][2];
                hessian.block<3, 3>(3, 6) = jacobiansLL[1][2];

                // Lower off diagonal
                hessian.block<3, 3>(3, 0) = jacobiansLL[1][0];
                hessian.block<3, 3>(6, 0) = jacobiansLL[2][0];
                hessian.block<3, 3>(6, 3) = jacobiansLL[2][1];
...
void VNCS::Sim3D::BendingForceField::addDForce(const sofa::core::MechanicalParams *mparams,
                                               ...
                                               const DataVecDeriv &xDiffVec)
{
    ...
    for (const auto &[element, hessian] : ranges::views::zip(m_elements, m_hessians)) {
        ...
        const auto xCenter = Eigen::Map<const Eigen::Vector3d>(&dxis[centerIndex][0]);
        const auto xRight = Eigen::Map<const Eigen::Vector3d>(&dxis[rightIndex][0]);

        Eigen::Matrix<VNCS::Real, 9, 1> dx;
        dx << xLeft, xCenter, xRight;

        Eigen::Matrix<VNCS::Real, 9, 1> df = mparams->kFactor() * hessian * dx;

        dfs[leftIndex][0] += df[0];
        dfs[leftIndex][1] += df[1];
        dfs[leftIndex][2] += df[2];

        dfs[centerIndex][0] += df[3];
        dfs[centerIndex][1] += df[4];
        dfs[centerIndex][2] += df[5];

        dfs[rightIndex][0] += df[6];
        dfs[rightIndex][1] += df[7];
        dfs[rightIndex][2] += df[8];
    }
}
...
const std::vector<Eigen::Matrix<Real, 9, 9>> &BendingForceField::hessians() const
{
    return m_hessians;
}
